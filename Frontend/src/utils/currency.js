...
/**
 * Strip markdown emphasis (* and **) from model output so literal asterisks never appear.
 * Call before formatNumbersInText so our own bold wrapping is applied after.
 */
export const stripMarkdownEmphasis = (text) => {
...
export const formatNumbersInText = (text, currencyOverride = null) => {
  ...
  const KNOWN_SYMBOLS = Array.from(new Set(CURRENCIES.map(c => c.symbol))).filter(Boolean);
  const escapeRegExp = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const endsWithSymbol = (beforeTrim, symbol) => {
    if (!beforeTrim || !symbol) return false;
    // For letter-only symbols (e.g. CHF, Rp), require a word boundary.
    if (/^[A-Za-z]+$/.test(symbol)) {
      return new RegExp(`${escapeRegExp(symbol)}\\b$`).test(beforeTrim);
    }
    return beforeTrim.endsWith(symbol);
  };
  const startsWithSymbol = (after, symbol) => {
    if (!after || !symbol) return false;
    const trimmed = after.replace(/^\s+/, '');
    if (/^[A-Za-z]+$/.test(symbol)) {
      return new RegExp(`^${escapeRegExp(symbol)}\\b`).test(trimmed);
    }
  ...
  while ((match = numberPattern.exec(text)) !== null) {
    ...
    if (isInTable(match.index, text)) {
      ...
      const beforeTrim = text.substring(0, match.index).replace(/\s+$/, '');
      const after = text.substring(match.index + numStr.length);

      // If the text already includes a currency symbol right next to the number
      // (e.g. "¥25,000" or "25,000 kr"), don't add another one (prevents "¥¥25,000").
      const hasPrefixCurrency = KNOWN_SYMBOLS.some(sym => endsWithSymbol(beforeTrim, sym));
      const hasSuffixCurrency = KNOWN_SYMBOLS.some(sym => startsWithSymbol(after, sym));
      const alreadyHasCurrency = hasPrefixCurrency || hasSuffixCurrency;
      ...
      // If model already had "**" after the number, skip them so we don't produce "****"
      if (after.startsWith('**')) {
        lastIndex = match.index + numStr.length + 2;
        continue;
      }
    } else {
